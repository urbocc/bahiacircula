  ...
  function mapPoints(points, layers) {
    ...
    for (var i in points) {
      ...
      if (point.Latitude !== '' && point.Longitude !== '') {
        var marker = L.marker([point.Latitude, point.Longitude], {icon: icon})
          ...
          (point['Foto'] ? ('<img src="' + point['Foto'] + '"><br>') : '') +
          point['Descripción']);

        if (layers !== undefined && layers.length !== 1) {
          marker.addTo(layers[point.Categoría]);
        }
    ...
    if (displayTable && columns.length > 1) {
      ...
      function updateTable() {
        var pointsVisible = [];
        for (i in points) {
          if (map.hasLayer(layers[points[i].Categoría]) &&
    ...
    for (i = 0; i < p.length; i++) {
      $.getJSON(p[i]['GeoJSON URL'], function(index) {
        return function(data) {
          ...
          if (p[index]['Descripción'] && p[index]['Descripción'] != '') {
            line.bindPopup(p[index]['Descripción']);
          }
